...
def post_process(message):
    if message.get('lora_meta') is None or message['lora_meta'].get('raw') is None:
        print(f'A message have no lora_meta.raw from Group ID:{message["grpid"]}, Node ID:{message["nid"]}')
        return message

    raw = base64.b64decode(message['lora_meta']['raw'])
    print(f'Group ID:{message["grpid"]}, Node ID:{message["nid"]}, type:{message["ntype"]}, desc.:{message["ndesc"]}, data:{message["data"]}, raw:{raw}')
...
if __name__ == '__main__':
    ...
    client = pyiotown.post.postprocess_common(args.url, 'Milesight EM500', post_process, args.user.strip(), args.token.strip())
    pyiotown.post.postprocess_loop_forever(client)
